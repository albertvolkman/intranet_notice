...
function meerkat(options) {
	...
	if(readCookie('meerkat') != 'dontshow')
	{
        ...
        // Hack added by Albert
		jQuery('#meerkat').before('<div id="meerkat-transparent"></div>');
	
		jQuery('#meerkat-wrap').css({'position':'fixed', 'width':'100%', 'height': settings.height}).css(settings.meerkatPosition,"0");
		jQuery('#meerkat-container').css({'background': settings.background, 'height': settings.height});

        // Hack added by Albert
        jQuery('#meerkat-transparent').css({'background': settings.background, 'height': settings.height});
		...
		if((settings.animation == "slide")&&(settings.showMeerkatOnLoad != "true")){			
			...
			jQuery(settings.close).click(function(){
				jQuery("#meerkat-wrap").slideUp(settings.animationSpeed, function() { bannerSlide(); });							
			});
			...
			jQuery(settings.close).click(function(){
				jQuery("#meerkat-wrap").fadeOut(settings.animationSpeed, function() { bannerSlide(); });								
			});
		...
		jQuery("#meerkat").hide("", bannerSlide());
	}
}